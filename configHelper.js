...
function generateLink()
{
  ...
  if(document.getElementsByName("cmode") != undefined && document.getElementsByName("cmode").length>0)
  {
    let cmode = document.querySelector("input[name=cmode]:checked").value;
    RequestParams['dm'] = cmode;
  }
...
function findDevice()
{
  ...
  window.open(result+"/pinger.html", '_blank');
}

function writetocontroller()
{

  let model = document.querySelector("input[name=model]:checked").value;
  let type = document.querySelector("input[name=type]:checked").value;

  let calling = "http://"+RequestParams['ip']+"/Con"+model+"/"+type;

  alert("I am updating the config of your controller to"+calling+ " after your OK. If you dont want this to happen, close the tab!");
  window.open(calling, '_blank');
}

function showhardware()
{
  let calling = "http://"+RequestParams['ip']+"/mem";
  window.open(calling, '_blank');
}
